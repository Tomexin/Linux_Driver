...
static struct class *seventh_drv_class;
static struct class_device	*seventh_drv_class_dev;
...
static int seventh_drv_open(struct inode *inode, struct file *file)
{
	...
	printk("seventh_drv_open() is called\n");
...
{
	printk("seventh_drv_write() is called\n");
	return 0;
}
...
static int seventh_drv_release(struct inode *inode, struct file *file)
{
...
static unsigned int seventh_drv_poll(struct file *file, struct poll_table_struct *wait)
{
	unsigned int mask = 0;
...
static int seventh_drv_init(void)
{
	...
	buttons_timer.function = buttons_timer_function;	//设置定时器函数
	add_timer(&buttons_timer);							//将定时器添加进内核
	//注册驱动程序
	major = register_chrdev(major, "seventh_drv", &seventh_drv_fops);

	//自动创建设备节点
	seventh_drv_class = class_create(THIS_MODULE, "seventh_drv");
	seventh_drv_class_dev = class_device_create(seventh_drv_class, NULL, MKDEV(major, 0), NULL, "seventh_drv");
...
static void seventh_drv_exit(void)
{
	...
	class_device_unregister(seventh_drv_class_dev);

	class_destroy(seventh_drv_class);
...
module_init(seventh_drv_init);//修饰注册驱动程序
module_exit(seventh_drv_exit);//修饰卸载驱动程序

/* 描述驱动程序的一些信息，不是必须的 */
MODULE_AUTHOR("xuxin_in_15#351");
MODULE_VERSION("0.1.0");
MODULE_DESCRIPTION("S3C2410/S3C2440 seventh Driver");
